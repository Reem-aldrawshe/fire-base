...
import 'package:serag_app/screens/add_private_khetma.dart';
import 'package:serag_app/screens/private_khetma_parts_screen.dart';
import 'package:serag_app/services/khetma_service.dart';

class KhetmaPrivateScreen extends StatelessWidget {
  const KhetmaPrivateScreen({super.key});

  @override
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (_) => KhetmaBloc(KhetmaService())..add(FetchKhetmas(isPublic: false)),
      child: const _KhetmaPrivateView(),
    );
  }
}

class _KhetmaPrivateView extends StatelessWidget {
  const _KhetmaPrivateView({super.key});
                  ...
                  child: BlocBuilder<KhetmaBloc, KhetmaState>(
                    builder: (context, state) {
                      if (state is KhetmaLoading) {
                        ...
                        return const Center(child: Text('لا توجد ختمات خاصة بعد'));
                            ...
                            return GestureDetector(
                              onTap: () {
                                Navigator.push(
                                  context,
                                  MaterialPageRoute(
                                    builder: (_) => PrivateKhetmaPartsScreen(khatma: khatma),
                                  ),
                                );
                              },
            ...
            child: GestureDetector(
              onTap: () async {
                await showModalBottomSheet(
                  ...
                  builder: (_) => AddPrivateKhetmaBottomSheet(
  onAdded: () {
    context.read<KhetmaBloc>().add(FetchKhetmas(isPublic: false));
  },
)
